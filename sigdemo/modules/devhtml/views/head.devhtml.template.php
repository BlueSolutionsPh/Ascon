...
function func_page_search(page){
	var frm_obj = document.getElementById("devhtml_search_form");
...
function On_BoxChecked(){
	for(var i=0; i<=arguments.length-1; i++) {
		...
		document.getElementById("devhtml_lumpdel").disabled=false;
	}
}
function Off_BoxChecked(){
	for(var i=0; i<=arguments.length-1; i++) {
		document.getElementById("chk_"+arguments[i]).checked=false;
		document.getElementById("devhtml_lumpdel").disabled=true;
	}
}
function check(){
	var chk_cnt=0;
	for(var i=0; i<=document.getElementsByName("chk_devhtml[]").length-1; i++) {
		if(document.getElementsByName("chk_devhtml[]")[i].checked==true){
			chk_cnt++;
		}
	}
	if(chk_cnt > 0){
...
function bulk_deletion(){
	...
	for(var i=0; i<=document.getElementsByName("chk_devhtml[]").length-1; i++) {
		if(document.getElementsByName("chk_devhtml[]")[i].checked==true){
			//hiddenì¬
			createhidden('chk_devhtml[]', document.getElementsByName("chk_devhtml[]")[i].value, formobj);
		}
	}
