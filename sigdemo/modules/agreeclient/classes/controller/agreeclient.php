...
class Controller_AgreeClient extends Controller_Template{
	/**
	 * Deselect client
	 */
	...
	public function action_index(){
		...
		$this->model = new Model_AgreeClient();
		switch($this->disp){
			case ACTION_INS:
				//Registration
	...
	private function disp_list(){
		...
		$all_agreeclient_cnt = $this->model->sel_cnt_client($this->search);

		//Pagination
		$pagination = Pagination::factory(array(
			'current_page'  => array('source' => 'query_string', 'key' => 'page'),
			'items_per_page' => MAX_CNT_PER_PAGE,
			'total_items'   => $all_agreeclient_cnt[0]->cnt,
		));

		//Data acquisition
		$this->search->offset = $pagination->offset;
		$arr_agreeclient = $this->model->sel_arr_client($this->search);

		//Set value to template
		$this->head_add = "head.agreeclient.template";
	...
	private function disp_ins(){
		if($this->act === "ins"){
			...
			$this->post = $this->session->get('agreeclient.ins_post');
			if($this->chk_token() && $this->ins_validation() && $this->ins()){
				//Discard session
				$this->session->delete('agreeclient.ins_post');
			...
			if($this->ins_validation()){
				//Store in session
				$this->session->set('agreeclient.ins_post', $this->post);

				//Template selection
				$this->template->set_filename("agreeclient.ins_conf.template");
			}
		} else {
			//If there is session data set as initial value
			if($this->session->get('agreeclient.ins_post')){
				$this->post = $this->session->get('agreeclient.ins_post');
				$this->session->delete('agreeclient.ins_post');
			}
		}
	...
	private function ins_validation(){
		...
		if($ret){
			$this->validation = Validation::factory($this->post)
				...
				->rule('max_total_cts_file_size', 'not_empty')
				->rule('max_total_cts_file_size', 'digit')
				->rule('max_total_cts_file_size', 'max_length', array(':value', '2'))
				->rule('max_total_cts_file_size', 'positive_int')
	...
	private function ins(){
		...
		$client->max_total_cts_file_size = $this->post["max_total_cts_file_size"] * 1024 * 1024 * 1024;
	...
	private function disp_up(){
		...
		if($this->act === "up"){
			//With data update
			$this->post = $this->session->get('agreeclient.up_post');
			if($this->chk_token() && $this->up_validation() && $this->up()){
				//Discard session
				$this->session->delete('agreeclient.up_post');
			...
			if($this->up_validation()){
				//Store in session
				$this->session->set('agreeclient.up_post', $this->post);

				//Template selection
				$this->template->set_filename("agreeclient.up_conf.template");
			}
		} else {
			...
			//If there is session data set as initial value
			if($this->session->get('agreeclient.up_post')){
				$this->post = $this->session->get('agreeclient.up_post');
				$this->session->delete('agreeclient.up_post');
			}
		}
