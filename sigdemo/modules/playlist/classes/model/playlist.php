...
class Model_Playlist extends Model
{
	 ...
	 * Post Return the key name of the data to be sent
	 */
	public function getPostInsDataKeys() {
		return array(
			"client_id",
			"playlist_name",
			"draw_tmpl_id",
	...
	function sel_cnt_playlist($search)
	{
		...
		if(isset($search->deliverymonth_kind) && $search->deliverymonth_kind !== ""){
			$deliverymonth_kind = $search->deliverymonth_kind;
		}
...
//		$query_str .= "	m_timezone.del_flag = 0 ";
		...
		//Add search condition (distribution type)
		if(isset($deliverymonth_kind)){
			$query_str .= "	t_playlist.deliverymonth_id = :deliverymonth_kind and ";
			$arr_bind_param[":deliverymonth_kind"] = $deliverymonth_kind;
		}
	 ...
	 * @return array					Acquisition record
	 */
	function sel_arr_playlist_overlap($search)
	{
		//Search condition
		$now = Request::$request_dt;
		if(!empty($search->arr_client_name)){
...
//		}
	 ...
	 * @return	array				Acquisition record
	 */
	public function sel_arr_id_name_playlist($playlist)
	{
		$query_str = "select ";
		$query_str .= "	client_id, ";
...
//		if(isset($sta_dt) && $sta_dt !== ""){
	 ...
	 * @return array					Acquisition record
	 */
	function sel_arr_movie_by_playlist_id_draw_area_id_dt_common($playlist_id, $draw_area_id, $sta_dt, $end_dt)
	{
		$query_str = "select ";
		$query_str .= "	playlist_movie.movie_id, ";
		$query_str .= "	playlist_movie.movie_name, ";
	...
	public function del_playlist($playlist)
	{
		...
		try{
			...
			$playlist_movie_rela->update_user = $playlist->update_user;
			$playlist_movie_rela->update_dt = $playlist->update_dt;
			$t_playlist_movie_rela->del_by_playlist_id($playlist_movie_rela);
			...
			$playlist_image_rela->update_user = $playlist->update_user;
			$playlist_image_rela->update_dt = $playlist->update_dt;
			$t_playlist_image_rela->del_by_playlist_id($playlist_image_rela);
			...
			$prog_playlist_rela->playlist_id = $playlist->playlist_id;
	...
	public function sel_arr_dev_by_playlist_id($playlist_id)
	{
		...
		if(isset($playlist_id->client_id)){
			$query_str .= "	t_prog_playlist_rela.client_id = :client_id and ";
		}
		$query_str .= "	t_prog_playlist_rela.del_flag = 0 ";
		$query_str .= "where ";
	 ...
	 */
	public function get_next_playlist_rela_id($search)
	{
		$playlist_rela_id = null;
		try{
			$t_playlist_rela = new Model_T_Playlist_Rela($this->db, $this->client_id);
			$playlist_rela_id = $t_playlist_rela->sel_cnt_playlist_rela($search);
		} catch(Exception $e){
			Kohana::$log->add(Log::ERROR, Kohana_Exception::text($e))->write();
			$playlist_rela_id = null;
		}
	...
	public function sel_arr_id_name_playlist_movie_rela($playlist)
	{
		...
		if(isset($playlist->playlist_id) && $playlist->playlist_id !== ""){
			$query_str .= " playlist_id = :playlist_id and ";
			$bindArr[":playlist_id"] = $playlist->playlist_id;
		}
		if(isset($playlist->single_flg) && $playlist->single_flg !== ""){
			$query_str .= " playlist_rela_id is NULL  and ";
		}
		if(isset($playlist->multi_flg) && $playlist->multi_flg !== ""){
			$query_str .= " playlist_rela_id is NOT NULL  and ";
		}
		$query_str .= "	del_flag = 0 ";
		$query_str .= "order by ";
		$query_str .= "	playlist_id, ";
	...
	/**
	 * Get playlist linkage information
	 *
	 * @return int		Playlist linkage information
	 */
	public function get_arr_playlist_rela($search)
	{
		$playlist_rela_id = null;
		try{
			$t_playlist_rela = new Model_T_Playlist_Rela($this->db, $this->client_id);
			$playlist_rela_id = $t_playlist_rela->sel_arr_playlist_rela($search);
		} catch(Exception $e){
			Kohana::$log->add(Log::ERROR, Kohana_Exception::text($e))->write();
			$playlist_rela_id = null;
		}
	...
	/**
	 * Check if there is a common playlist within the time period and return an existing list
	 *
	 * @param array $data Post data
	 * @return array boolean
	 */
	public function existCommonListInPeriod($data) {
		$query = 'select ' .
			'sex_id, ' .
			'timezone_id, ' .
			'sta_dt, ' .
			...
			'deliverymonth_id = :deliverymonth_id and ' .
			'sta_dt = :sta_dt and ' .
			'end_dt = :end_dt';
		$db = DB::query(Database::SELECT, $query);
		$db->parameters(array(
			':deliverymonth_id' => $data['deliverymonth_id'],
			':sta_dt'           => $data['sta_dt'] . " 00:00:00",
			':end_dt'           => $data['end_dt'] . " 23:59:59",
		));
		$results = $db->execute($this->db, true);
		...
		foreach ($results as $result) {
			if (isset($check[$result->sex_id][$result->timezone_id])) {
				$check[$result->sex_id][$result->timezone_id] = true;
			}
		}
		return $check;
	}
