    ...
    <div id="main">
      ...
      <div class="box_01">
        ...
        <div class="content">
          ...
          <?php echo Form::hidden("arr_time_zone[]", $post["arr_time_zone"]) ?>
          <?php echo Form::hidden("arr_sex[]", $post["arr_sex"]) ?>
          <?php echo Form::hidden("prog_name", time()) ?>
<?php for ($m = 0;$m <= 1;$m++): ?>
<?php for ($n = 1;$n <= 3;$n++): ?>
          <?php echo Form::select("arr_movie" . $m . $n . "[]", null, null, array("id" => "playlist_ins_arr_movie" . $m . $n, "multiple" => "multiple", "style" => "visibility:hidden;height:0")) ?>
<?php endfor; ?>
<?php endfor; ?>
          ...
          <div class="hr_wrapper_01">
             <table cellspacing="1" cellpadding="3" border="0">
               ...
               <dd>
                 <?php echo Form::select("cp_playlist_id", $arr_all_playlist,  HTML::replace_empty_array_value($post, "cp_playlist_id"),array("id" => "playlist_ins_cp_playlist_id")) ?>
          ...
          <div class="hr_wrapper_01">
            <?php echo Form::open(null, array("id" => "playlist_search_form")) ?>
              <table cellspacing="1" cellpadding="3" border="0" class="searchForm">
                ...
                <tr><td>Movie title name</td><td><?php echo Form::input("movie_name", HTML::replace_empty_array_value($post, "movie_name"), array("id" => "playlist_ins_movie_name", "maxlength" => "60", "class" => "input350")) ?></td></tr>
                <tr><td>Movie tag</td><td><?php echo Form::select("movie_tag_1", $arr_all_tag, HTML::replace_empty_array_value($post, "movie_tag_1"), array("id" => "playlist_ins_movie_tag_1")) ?></td>
                    <td colspan="2"><?php echo Form::button(NULL, "Search", array("id" => "playlist_ins_search", "type" => "submit", 'class'=>'btn3')) ?></td></tr>
              </table>
...
<?php $key = key($movie);?>
                            <td class="ui-state-default drop-target" data-name="<?php echo $m . $n;?>" data-id="<?php echo $key;?>"><span class="value"><?php echo $movie[$key]['movie_name'];?></span><span class="close">×</span></td>
<?php else: ?>
...
<?php endfor; ?>
              ...
              <br /><div class="btn_area_02">
                <?php echo Form::button(NULL, "Return", array("id" => "playlist_ins_back", 'class'=>'btn1 btn_l')) ?>
                <?php echo Form::button(NULL, "Registration", array("id" => "playlist_ins_submit", "type" => "submit", 'class'=>'btn1 btn_r')) ?>
              </div>
            <?php echo Form::close() ?>
            <div class="clear"></div>
          </div>
