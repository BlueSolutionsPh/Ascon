...
function func_page_search(page){
	var frm_obj = document.getElementById("html_search_form");
...
function On_BoxChecked(){
	for(var i=0; i<=arguments.length-1; i++) {
		...
		document.getElementById("html_lumpdel").disabled=false;
	}
}
function Off_BoxChecked(){
	for(var i=0; i<=arguments.length-1; i++) {
		document.getElementById("chk_"+arguments[i]).checked=false;
		document.getElementById("html_lumpdel").disabled=true;
	}
}
function check(){
	var chk_cnt=0;
	for(var i=0; i<=document.getElementsByName("chk_html[]").length-1; i++) {
		if(document.getElementsByName("chk_html[]")[i].checked==true){
			chk_cnt++;
		}
	}
	if(chk_cnt > 0){
...
function bulk_deletion(){
	...
	for(var i=0; i<=document.getElementsByName("chk_html[]").length-1; i++) {
		if(document.getElementsByName("chk_html[]")[i].checked==true){
			//hiddenì¬
			createhidden('chk_html[]', document.getElementsByName("chk_html[]")[i].value, formobj);
		}
	}
