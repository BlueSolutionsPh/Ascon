...
function func_page_search(page){
	var frm_obj = document.getElementById("commontext_search_form");
...
function On_BoxChecked(){
	for(var i=0; i<=arguments.length-1; i++) {
		...
		document.getElementById("commontext_lumpdel").disabled=false;
	}
}
function Off_BoxChecked(){
	for(var i=0; i<=arguments.length-1; i++) {
		document.getElementById("chk_"+arguments[i]).checked=false;
		document.getElementById("commontext_lumpdel").disabled=true;
	}
}
function check(){
	var chk_cnt=0;
